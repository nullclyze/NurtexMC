...
Здесь мы напишем простого **Minecraft** бота, задачей которого будет **просто подключиться** к серверу (в данном примере будет рассматриваться подключение к локальному серверу используя версию `1.21.11`).
...
Давай напишем бота, задачей которого будет подключиться к серверу и 5 раз написать определённое сообщение в чат.
...
Тут мы напишем минимальную программу с использованием роя. Её задачей будет запустить сразу несколько ботов на сервер и просто ожидать закрытия.
